        ...
        for line in self:
            ...
            if line.month:
                ...
                if florence_fp_id:
                    ...
                    domain2 = [
                        "&", "&", "&",
                        ("parent_id", "!=", florence_fp_id.id),
                        ("date", ">=", str(line.name.name) + "-" + str(line.month) + "-" + "01"),
                        ("date", "<=", str(line.name.name) + "-" + str(line.month) + "-" + str(last_day)),
                        ("is_single_cost", "=", True)
                    ]
                    florence_fp_lines_single_cost = self.env["florence.financial.plan.line"].search(domain2)
                    line.monthly_total += sum([
                        fp_line.approved for fp_line in florence_fp_lines_single_cost
                    ]) if len(florence_fp_lines_single_cost) > 0 else .0
