...
class Invoice(models.Model):
    ...
    def remove_note_section(self, vals):
        if 'LineItems' in vals:
            vals.get('LineItems')[:] = [item for item in vals.get('LineItems') if
                                        'AccountCode' in item and item['AccountCode'] != None and item['Quantity'] != 0.0]
        return vals
    ...
    def exportInvoice_cron(self):
        ...
        invoice_id = self.env['account.move'].search([('xero_invoice_id', '=', False),
                                                      ('state', '=', 'posted'),
                                                      ('create_date', '>', xero_config.export_record_after)])
...
class InvoiceLine(models.Model):
    ...
    _sql_constraints = [('check_amount_currency_balance_sign', 'CHECK(1=1)', 'Contriants Error fix for migrate')]
